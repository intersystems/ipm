...
- Omit issue and PR numbers in in-code documentation. They are clutter once merged, especially for fixes within an unreleased feature.
...
- Changelog entries must be included for all PRs except those fixing/modifying an unreleased feature and must reference the issue number being fixed, if one exists.
