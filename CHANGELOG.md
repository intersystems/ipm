...
### Fixed
- Fix `sync -test` never running the tests for a test file that an earlier `sync` (without `-test`) already applied. The earlier sync commits the file's hash, so change detection would not look at it again and the edit stayed untested. Such files are now flagged, reported as `N test file(s) synced without running tests; they will run on the next sync with -test.`, and forced into the next sync with `-test` regardless of their hash — which reports `Running tests for N file(s) synced earlier without -test.` and clears the flag.
- Fix `sync -test` running a changed unit test once per `<UnitTest>` resource that shares its directory. Several `<UnitTest>` resources may point at one test tree and differ only by `Package`; changed test classes were grouped by resource directory, so a single edit ran in every resource sharing it (six resources ran the same test six times). Each changed class is now assigned to the one resource whose `Package`/`Class` covers it, and a changed class outside that resource's package is compiled so a newly added test in a shared directory is picked up and run.
- #1224: Fix `sync` reporting `Nothing to sync.` after a file that failed to load or compile is fixed by reverting it. A failed sync commits no hashes, so restoring the previously-synced content made the file match its baseline again while the namespace still held whatever the failed attempt left behind. Paths from a failed sync are now flagged and forced through the next sync regardless of their hash, and reported as `Retrying N file(s) from a previous failed sync.`
