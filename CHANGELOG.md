...
- #1137: Python packages from both `<PythonWheel>` resources and a module's `requirements.txt` are now
staged and copied into `mgr/python` rather than installed there directly, so package files inherit that
directory's permissions. On Windows this prevents packages
from being unreadable to non-elevated processes, which surfaced as a `PermissionError` when using
ORAS registries. Reinstalling IPM repairs an instance whose packages are already unreadable.
